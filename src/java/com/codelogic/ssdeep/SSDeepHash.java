...
public class SSDeepHash {
    ...
    SSDeepHash(String ssDeepHash) {
        ...
        if (firstColonIndex <= 0 || secondColonIndex <= 0 || firstColonIndex == ssDeepHash.length() - 1 || secondColonIndex == ssDeepHash.length() - 1 || firstColonIndex == secondColonIndex) {
            throw new IllegalArgumentException("Invalid SSDeep hash provided '" + ssDeepHash + "'.");
        }
     ...
     * a slightly modified Levenshtein distance calculation which is used by the ssdeep application when calculating
     ...
     * Copyright (C) 2016 Russell Francis &lt;russell.francis@gmail.com&gt;
     */
